...
export default class Actions {
  ...
  showAction(action, show, input, back) {
    ...
    setTimeout(() => (show.style.display = "block"), 300);
    setTimeout(() => {
  ...
  hideAction() {
    ...
    setTimeout(() => {
      ...
      customersVar.container
        .querySelector(".customers-hr")
    ...
    setTimeout(() => {
      budget.style = defaultStatus;
      budget.classList.remove("show");

      listElem.style = defaultStatus;
      listElem.classList.add("none");

      purchasesVar.container.style = defaultStatus;
      purchasesVar.container.classList.remove("show");

      controlVar.container.style = defaultStatus;
      controlVar.container.classList.remove("show");

      customersVar.container.style = defaultStatus;
  ...
  itemPriceTotal() {
    if (this.variables.materials.value) {
      ...
      this.variables.price.textContent = `$${(
        materialPrice * materialQuantity
      ).toLocaleString()},00 - Total`;
    }
  }
  ...
  cleanPurchaseFields() {
    ...
    this.variables.ci.style = defaultStatus;
...
buttons.budget.onclick = () => {
  ...
  new Actions(buttons).showAction(
    actionsContainer,
    budget,
    input,
    buttons.back
...
purchasesVar.back.addEventListener("click", (e) => {
  new Actions(purchasesVar).showPurchaseResult(
    purchasesVar.namelist.parentNode,
    purchasesVar.seeMaterials
  );
});
purchasesVar.namelistBtn.addEventListener("click", (e) => {
  ...
  setTimeout(() => {
    purchasesVar.paySuccess.children[1].style = defaultStatus;
  }, 3000);
});
...
controlVar.ci.addEventListener("keyup", (e) =>
  new Actions(controlVar).validateCI()
);
controlVar.btn.addEventListener("click", (e) => {});

// VALIDAR CI DE BUSCAR CLIENTES
customersVar.ci.addEventListener("keyup", (e) =>
  new Actions(customersVar).validateCI()
);
